...
// grok 3
// adAction.js
// Assuming MongoDB Atlas Search with AI integration is set up
// adAction.js
// adAction.js
...
export async function getPaginatedAds(req, res) {
  try {
    ...
    // Step 1: Search by area if provided
    if (area) {
      const areaParts = area.split(",").map((part) => part.trim()); // Split and trim input
      ...
      // If a service is provided, filter by it
      if (service) {
        searchQuery.service = { $regex: new RegExp(service, "i") };
      }
      ...
      ads = ads[0].data; // Extract the data array
    }
    // Step 2: If no area or no results from area, and service is provided, use AI-powered search
    else if (service) {
      const aiSearchPipeline = [
        {
          $search: {
            ...
            text: {
              ...
              fuzzy: { maxEdits: 2 }, // Allow flexibility in matching
            },
          },
        },
      ...
      ads = await AD.aggregate(aiSearchPipeline).exec();
      total = ads[0].metadata.length ? ads[0].metadata[0].total : 0;
      ads = ads[0].data; // Extract the data array
    }
    // Step 3: If no area and no service, fetch all non-blocked ads
    else {
      ads = await AD.aggregate([
        { $match: { block: false } }, // Exclude blocked ads
        {
    ...
    // Step 4: Return the response in your original format
    return {
      success: true,
      data: JSON.parse(
        JSON.stringify({
