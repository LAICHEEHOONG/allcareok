...
//   const { page, limit } = request; // Default to page 1 and limit 10
...
//     const photos = await AD.find({}, { photo: 1 }) // Select only the `photo` field
//       .skip(skip)
//       .limit(limit)
//       .lean(); // Optimize for read operations

//     // Count total photos for pagination metadata
//     const totalPhotos = await AD.countDocuments();
...
// export async function aggregateAdsData(request) {
//   const { matchCriteria, groupByField, sortField, limit } = request;
...
//     const pipeline = [
//       {
//         $match: matchCriteria || {}, // Match documents based on criteria
//       },
//       {
//         $group: {
//           _id: `$${groupByField}`, // Group by the specified field
//           count: { $sum: 1 }, // Count the number of documents
//           data: { $push: "$$ROOT" }, // Include the full documents
//         },
//       },
//       {
//         $sort: { [sortField]: -1 }, // Sort by the specified field in descending order
//       },
//     ];

//     if (limit) {
//       pipeline.push({ $limit: limit }); // Limit the number of results
//     }

//     // Perform the aggregation
//     const result = await AD.aggregate(pipeline).exec();

//     return { success: true, data: result };
//   } catch (error) {
//     console.error("Error aggregating ads data:", error.message);
//     return { success: false, message: error.message };
//   }
// }
