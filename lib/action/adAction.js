...
//       const { town, city, state } = areaData.area;
//       return [town, city, state].filter(Boolean).join(", ");
//     });

//     let country = areaDataList.map((areaData) => {
//       return areaData.area.country;
//     });
//     const filteredCountry = [
//       ...new Set(
//         country
//           .filter((item) => item && item.trim()) // Remove empty, null, and undefined values
//           .map((item) => item.toLowerCase()) // Convert to lowercase for case-insensitive comparison
//       ),
//     ];

//     // Remove duplicates
//     const uniqueFormattedAreas = [...new Set(formattedAreas)].filter(Boolean);

//     let combinedAreas = [...uniqueFormattedAreas, ...filteredCountry];

//     // Sorting based on query match
//     combinedAreas.sort((a, b) => {
//       const scoreA = a.toLowerCase().includes(query.toLowerCase()) ? 1 : 0;
//       const scoreB = b.toLowerCase().includes(query.toLowerCase()) ? 1 : 0;
...
//       const locationParts = [town, city, state].filter(Boolean).join(", ");

//       if (locationParts) {
//         // If there's a town, city, or state, return only those (exclude country)
//         if (!uniqueAreas.has(locationParts.toLowerCase())) {
//           uniqueAreas.add(locationParts.toLowerCase());
//           results.push({ location: locationParts });
//         }
//       } else if (country && !uniqueAreas.has(country.toLowerCase())) {
//         // If there’s only a country, return it separately
//         uniqueAreas.add(country.toLowerCase());
//         results.push({ location: country });
//       }
//     });
//     console.log(results)
...
export async function getPaginatedAds(req, res) {
  try {
    ...
    // If a service is provided, filter by it (checking if the string exists in the service array)
    if (service) {
