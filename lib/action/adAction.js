...
//   try {
//     // Construct a query to match any area fields (country, state, city, town)
//     const searchQuery = {
//       $or: [
//         { "area.country": { $regex: new RegExp(query, "i") } },
//         { "area.state": { $regex: new RegExp(query, "i") } },
...
//     // Fetch distinct area values from existing ads
//     const areas = await AD.aggregate([
//       { $match: searchQuery },
//       {
//         $group: {
//           _id: null,
//           countries: { $addToSet: "$area.country" },
//           states: { $addToSet: "$area.state" },
//           cities: { $addToSet: "$area.city" },
//           towns: { $addToSet: "$area.town" },
//         },
//       },
//       {
//         $project: {
...
//     if (areas.length === 0) {
//       return { success: true, data: { countries: [], states: [], cities: [], towns: [] } };
//     }

//     // Function to process and remove duplicates after trimming and converting to lowercase
//     const formatUnique = (arr) => [...new Set(arr.map((item) => item?.trim().toLowerCase()))];

//     // Convert all area values to lowercase, trim spaces, and remove duplicates
//     const formattedAreas = {
//       countries: formatUnique(areas[0].countries),
//       states: formatUnique(areas[0].states),
//       cities: formatUnique(areas[0].cities),
//       towns: formatUnique(areas[0].towns),
//     };

//     return { success: true, data: formattedAreas };
//   } catch (error) {
...
//     return {
//       success: true,
//       data: areas.length ? areas[0] : { countries: [], states: [], cities: [], towns: [] },
//     };
//   } catch (error) {
//     console.error("Error fetching area suggestions:", error);
//     return { success: false, message: error.message };
...
//         $addFields: {
//           isTopRanking: {
//             $cond: { if: { $gt: ["$topRanking", currentDate] }, then: 1, else: 0 }
//           }
//         }
//       },
//       {
//         $sort: { isTopRanking: -1, topRanking: -1, views: -1 } // Prioritize valid top-ranking ads, then by views
//       },
//       {
//         $facet: {
//           metadata: [{ $count: "total" }],
